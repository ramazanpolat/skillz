...
<!--
This skill is a MODIFIED version of herdr's agent SKILL.md.

Original:   https://github.com/ogulcancelik/herdr  (SKILL.md)
Copyright:  (c) herdr authors.  Licensed under AGPL-3.0-or-later.

Modifications by Ramazan Polat, 2026-06-30:
  - "discover yourself": self-identify via $HERDR_PANE_ID and `herdr pane current`;
    warn that `focused: true` / `--current` follow the user's UI focus, not the
    calling pane (so splits land in the wrong workspace with multiple agents).
...
your own pane — the one this shell runs in — is given to you two ways that always
agree: the `HERDR_PANE_ID` env var, and `herdr pane current`.

```bash
echo "$HERDR_PANE_ID"   # your pane id, already in the environment
herdr pane current      # json; your pane id is at result.pane.pane_id
```

do not identify yourself by the `focused: true` entry in `pane list`, and do not
use the `--current` flag to mean "my pane". `focused` and `--current` follow the
**user's ui focus** — whichever pane the user is looking at right now — not the
pane your shell runs in. with several agents live in different workspaces at once,
the user glancing at a sibling agent flips focus over there, so any split or read
you key off focus lands in the wrong workspace. `HERDR_PANE_ID` and `pane current`
are the only self-identification that stays correct.

`pane list` then shows everyone — your neighbors included:
...
split your pane to the right and keep focus on your current pane. pass your own
pane id — `$HERDR_PANE_ID` or the one from `pane current` — as the target, not
`--current`:
...
- `focused: true` in `pane list` and the `--current` flag both follow the user's ui focus, not the calling shell. to act on your own pane, resolve it with `HERDR_PANE_ID` or `pane current` and pass that id explicitly — otherwise, with multiple agents in different workspaces, the action can land in whichever pane the user is currently viewing.
